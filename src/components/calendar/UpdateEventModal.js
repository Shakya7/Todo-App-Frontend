  ...
  return (
    <div className='fixed top-0 left-0 w-screen h-screen backdrop-blur-sm z-10 flex justify-center items-center'>
        <div className="w-4/5 lg:w-1/2 xsm:min-w-fit sm:h-auto bg-neutral-700 rounded-md flex flex-col justify-center items-center sm:items-start p-4 relative gap-4 m-5">
            ...
            <h1 className="text-updateTodoText xsm:text-xl text-white font-nunito">Update Event</h1>
            ...
            <div className="flex w-full gap-2 text-gray-400 flex-col msm:flex-row">
                <div className="flex gap-2 flex-col xxsm:flex-row">
                    <div className="flex flex-col items-start gap-2 text-filter xsm:text-base">
                        ...
                        }} defaultValue={currentEventState.start_date} style={{colorScheme:"dark"}} className="bg-zinc-800 rounded-md px-1.5 py-1" type="date"/>
                    </div>
                    <div className="flex flex-col items-start gap-2 text-filter xsm:text-base">
                        ...
                        }} defaultValue={currentEventState.end_date} style={{colorScheme:"dark"}} className="bg-zinc-800 rounded-md px-1.5 py-1" type="date"/>
                    </div>
                </div>
                <div className="flex gap-2">
                    <div className="flex flex-col items-start gap-2 text-filter xsm:text-base">
                        ...
                        }) } defaultValue={currentEventState.start_time} style={{colorScheme:"dark"}} className="bg-zinc-800 rounded-md w-full px-1.5 py-1" type="time"/>
                    </div>
                    <div className="flex flex-col items-start gap-2 text-filter xsm:text-base">
                        ...
                        }) } defaultValue={currentEventState.end_time} style={{colorScheme:"dark"}} className="bg-zinc-800 rounded-md w-full px-1.5 py-1" type="time"/>
                    </div>
                </div>
            </div>
            <div className="flex justify-center items-center gap-2">
                ...
                <p className="text-zinc-400 text-filter xsm:text-xs">{currentEventState.start_date===currentEventState.end_date?`This event will take place on ${currentEventState.start_date?new Date(currentEventState.start_date).toLocaleDateString('en-us',{ year: 'numeric', month: 'short', day:"numeric" }):"00-00-0000"} 
            ...
            <div className="flex flex-col items-start w-full gap-2 text-filter xsm:text-base">
                ...
                <input onChange={e=>setCurrentEventState({
                    ...currentEventState,
                    link:e.target.value
                })} defaultValue={currentEventState.link} type="text" className="text-gray-400 w-full bg-zinc-800 rounded-md px-2 py-1 outline-none"/>
            </div>
